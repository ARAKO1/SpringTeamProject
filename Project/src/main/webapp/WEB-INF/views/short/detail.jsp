			...
			<form action="/short/detail" method="post" name="reserveForm2">
				<table class="table" id="table">
					...
					<tr>
						...
						<th>예약</th>
						<td><input type="submit" name="reser()" value="예약" /> <input type="hidden"
...
<%-- <div class="container col-sm-6 fixed-bottom"
	...
	<div class="row card" style="background-color: orange;">
		...
		<form name="reserveForm2" action='shortTerm_payment2'>
			<nav class="container navbar navbar-expand-sm navbar-dark bottom"
				style="height: 70px;">
				<div class="col-sm-9" style="color: white;">
					<div class="row">
						<h4 class="col">총 결제금액</h4>
						<h4 class="float-right">0원</h4>
					</div>
				</div>
				<select id="insurance_seq" name="insurance_seq"
						class="form-control">
							<c:forEach var="insurance" items="${insuranceList}" varStatus="i">
								<option value="${insurance.insurance_seq}">${insurance.insurance_name}</option>
							</c:forEach>
					</select>
				<div class="col-sm-3">
					<div class="row">
						...
						<button type="reset" class="row btn col-sm-5 border"
							style="color: white; font-size: 15px;" onclick="reset()">초기화</button>
						&nbsp;&nbsp;&nbsp;&nbsp;
